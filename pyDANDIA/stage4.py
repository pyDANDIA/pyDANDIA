...
def resample_image(new_images, reference_image_name, reference_image_directory, reduction_metadata, setup,
    ...
    for new_image in new_images:
        ...
        #data_image = gaussian_filter(data_image,3)
        ...
        shifted = tf.warp(data_image, inverse_map=model_final.inverse, output_shape=data_image.shape, order=5, mode='constant',
                         cval=np.median(data_image), clip=False,preserve_range=False)

        #shifteds = iterate_on_resampling(ref_sources,reference_image, data_image,reduction_metadata,row_index)
        #shifted2 = shifteds[2]
        #import matplotlib.pyplot as plt
        #plt.imshow(shifted2-shifted,vmin=-100,vmax=100)
        #plt.show()
        #shifted = shifted2
        #shifted = astroalign.register(data_image,reference_image)
        #transf, (s_list, t_list) = astroalign.find_transform(data_image,reference_image)
        #shifted2 = tf.warp(data_image, inverse_map=model_final2, preserve_range=True)
        shifted_mask = tf.warp(mask_image, inverse_map=model_final.inverse, preserve_range=True)
        master_mask += shifted_mask
        #import matplotlib.pyplot as plt
        #plt.imshow(shifted-reference_image,vmin=np.percentile(shifted-reference_image,33),vmax=np.percentile(shifted-reference_image,66))
        #plt.show()
        #import pdb; pdb.set_trace()
        if mask_extension > -1:
            shifted_mask = tf.warp(mask_image, inverse_map=model_final.inverse, preserve_range=True)
