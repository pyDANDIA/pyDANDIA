...
def mask_datapoints_by_image_stamp(photometry, reduction_metadata, bad_data_index, error_code):
    ...
    for s in stamps:
        ...
        print('Stamp: ',stamp_dims['X_MIN'].value, reduction_metadata.star_catalog[1]['x'], type(reduction_metadata.star_catalog[1]['x'].data))
        affected_stars = np.where( (reduction_metadata.star_catalog[1]['x'].data >= stamp_dims['X_MIN'].data) & \
                        ...
                        (reduction_metadata.star_catalog[1]['y'].data < stamp_dims['Y_MAX'].data) )[0]
