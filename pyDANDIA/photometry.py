...
def run_psf_photometry_on_difference_image(setup,reduction_metadata,log,ref_star_catalog,
	...
	error_ref_flux = ref_star_catalog[j,6]
 	...
 	psf_parameters[2] = xstar
        ...
        psf_extended = psf_image
	psf_convolve = convolution.convolve_image_with_a_psf(psf_extended, kernel, fourrier_transform_psf=None, fourrier_transform_image=None,
	...
	good_fit = True
	    ...
	    mask = dist<radius**2
            ...
            #xweight = np.abs(psf_fit[mask].ravel())**0.5
            #import pdb; pdb.set_trace()
...
#		    plt.show()
	    ...
	    #flux_err = ((flux_err*intensities[0])**2+(flux*cov[0][0]**0.5)**2)**0.5
            flux = flux*intensities[0]
	    flux_err *= intensities[0] 
            (back,back_err) = (intensities[1],cov[1][1]**0.5)
	    psf_parameters[0] = intensities[0]
	    psf_model.update_psf_parameters( psf_parameters)
	    psf_image = psf_model.psf_model(Y_grid, X_grid, psf_parameters)
	    psf_extended = psf_image
	    psf_convolve2 = convolution.convolve_image_with_a_psf(psf_extended, kernel,)

	    flux = np.sum(psf_convolve2)
            flux_err = (np.sum(flux))**0.5
	    ...
	    (mag, mag_err) = convert_flux_to_mag(flux_tot/300., flux_err_tot/300.)
