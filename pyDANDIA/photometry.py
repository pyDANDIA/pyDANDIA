...
def run_stage6(setup):
    ...
    time = []
    exposures_id = []
    photometric_table = np.zeros((len(new_images),len(starlist),16))
...
def photometry_on_the_difference_image(setup, reduction_metadata, log, star_catalog,difference_image, psf_model, sky_model, kernel,kernel_error, ref_exposure_time):
    '''
    Find the appropriate kernel associated to an image
    :param object reduction_metadata: the metadata object
    :param string kernels_directory: the path to the kernels
    ...
    column_names = ('exposure_id','star_id','reference_mag','reference_mag_err','reference_flux','reference_flux_err','diff_flux','diff_flux_err','magnitude','magnitude_err',
                    'phot_scale_factor','phot_scale_factor_err','local_background','local_background_err','residual_x','residual_y')
