...
def run_stage1(setup, rerun_all=None):
    ...
    Output: updates the 'images_stats' layer in the metadata file
    """
    ...
    # Find any new images that stage 1 needs to process
    ...
    full_path_to_images = [os.path.join(setup.red_dir, 'data', i) for i in images]

    # If no new images are available for stage 1 to process, exit
    if len(images) != 0:
        log.info('Analyzing ' + str(len(images)) + ' images')
    else:
        logs.ifverbose(log,setup,'No new images for stage1 to process. Exiting stage1.')
        status = 'OK'
