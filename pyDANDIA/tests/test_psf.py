...
def test_coadd_stamps():
    """Function to test the coaddition of stamp images to make a master stamp"""

    setup = pipeline_setup.pipeline_setup({'red_dir': TEST_DATA})

    log = logs.start_stage_log( cwd, 'test_coadd_stamps' )

    stamp_dims = (20,20)
    xcen = int(stamp_dims[0]/2)
    ycen = int(stamp_dims[1]/2)
    nstamps = 20
    stamps = []

    for i in range(0,nstamps,1):
        s = Cutout2D(np.ones(stamp_dims), (xcen, ycen), stamp_dims, copy=True)
        stamps.append(s)

    (master_stamp, master_stamp_var) = psf.coadd_stamps(setup, stamps, log,
                                                        diagnostics=True)

    hdu = fits.PrimaryHDU(master_stamp.data)
    hdulist = fits.HDUList([hdu])
    hdulist.writeto(os.path.join(TEST_DATA,'master_stamp.fits'),
                                 overwrite=True)
    hdu = fits.PrimaryHDU(master_stamp_var.data)
    hdulist = fits.HDUList([hdu])
    hdulist.writeto(os.path.join(TEST_DATA,'master_stamp_var.fits'),
                                 overwrite=True)

    assert master_stamp.data.all() == 1.0
    assert master_stamp_var.data.all() == 0.0
