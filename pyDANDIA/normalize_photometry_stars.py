...
def normalize_star_datasets(params, xmatch, quad_phot, survey_time_bins,
    ...
    lc = load_norm_field_photometry_for_star_idx(params, field_idx,
                                                xmatch, quad_phot,log=log)
    for dset, lc_data in lc.items():
        log.info(dset+': '+str(len(lc_data)))
    ...
    for f in filter_list:
        ...
        log.info('Binned primary reference lightcurve '+pri_ref_code)
        ...
        for dset in lc.keys():
            ...
            if f in dset:
                if dset != pri_ref_code:
                    ...
                    log.info('Binned dataset lightcurve '+dset)
                    ...
                    log.info(dset+' measured offset: '+str(offset)+'+/-'+str(offset_error))

                    # Apply the offset to correct the unbinned lightcurve from the
                    # dataset:
                    if diagnostics: data_orig = copy.deepcopy(data)
                    data = apply_dataset_offset(data, offset, offset_error, log)
...
def measure_dataset_offset(binned_pri_ref_lc, binned_lc, log, plot_file=None):
    ...
    log.info('Identified '+str(len(select))
